...
resource "proxmox_vm_qemu" "k3s_server" {
  ...
  cpu {
    ...
    type    = "host" # Passes through CPU flags for better performance
  }
...
# GPU Node
resource "proxmox_vm_qemu" "k3s_gpu_node" {
  ...
  # GPU specific hardware settings
  pci {
    id      = 0
    mapping_id = "igpu"
    rombar  = true
  ...
  # Ensure machine is q35 for PCI passthrough
  machine = "q35"
...
k3s_cluster:
  children:
    server:
      hosts:
        ${proxmox_vm_qemu.k3s_server.default_ipv4_address}:
    agent:
      hosts:
        ${proxmox_vm_qemu.k3s_gpu_node.default_ipv4_address}:
%{ for node in proxmox_vm_qemu.k3s_node ~}
        ${node.default_ipv4_address}:
%{ endfor ~}
  vars:
    ansible_user: "ubuntu"
    k3s_version: "v1.34.3+k3s1"
    token: "mysecretk3stoken" 
    api_endpoint: "${proxmox_vm_qemu.k3s_server.default_ipv4_address}"
EOT
}
